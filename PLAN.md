...
| **Salud** | 136 pruebas en verde (98 servidor + 38 móvil) · TypeScript limpio en `server/` y `mobile/` · empaqueta para Android · `expo prebuild` acepta los plugins de LiveKit |
...
| 12.12 Banner de recompensa premium | ✅ | Pedido por el usuario el 2026-09-11 con una referencia de estética negra y oro. **Dibujado, no una lámina**: placa, bordes metálicos, resplandor y partículas son vistas y degradados, y la única imagen es la moneda. Eso permite cambiar el color por nivel tocando una paleta (`reward-theme.ts`) en vez de exportar un juego de PNG por cada uno, y se ve nítido en cualquier pantalla. Jerarquía de tres tamaños —nombre > multiplicador > cantidad— y el multiplicador en columna aparte, porque pegado al nombre el ojo lo lee como una frase y se pierde el golpe. El ×N lleva degradado metálico real con `MaskedView`. Entrada por fases: ráfaga, moneda, placa, nombre y por último la cifra. La moneda sale de la referencia con la corona sustituida por la L de Livora, que es lo que la hace nuestra. Se borran las cuatro láminas anteriores: 2,7 MB menos |
| 12.13 Catálogo de sesenta regalos con arte | ✅ | Veinte escalones de precio de 1 a 20.000 monedas, **tres regalos en cada uno**: con una sola opción por precio la elección es el precio, y con tres cada cual manda el que le pega. Las 54 ilustraciones que faltaban salen de una lámina del usuario, recortadas por rejilla y con el negro pasado a transparencia. Solo queda con emoji el trono del club. El retorno lo sigue decidiendo el precio, así que la prueba pasa a vigilar **la escalera** y no una lista de sesenta nombres que nadie mantendría |
| 12.14 Foto de perfil, estado, ojo en la clave y panel de agencia | ✅ | Pedido el 2026-09-11 por quien está probando. **Foto:** se elige de la galería, se encoge a 512 px y se sube en base64; se guarda en disco con el nombre sacado del hash del contenido, que evita duplicados y hace que no se pueda adivinar la ruta de la foto de otro. Se sube al momento, no al guardar, porque tras pedir permiso y elegir, acordarse de guardar es donde la mitad sale creyendo que se cambió. **Estado:** una línea aparte de la bio —qué haces ahora, no quién eres—, que mezclados dejan un campo que nadie actualiza. **Ojo en la contraseña:** escribirla a ciegas en un teléfono es donde más gente se queda fuera, y el error que sale después no dice que fuera una errata. **Agencia:** cada anfitrión con sus horas de hoy y su barra de meta, ordenados por lo que llevan hoy; lo acumulado dice quién fue bueno el mes pasado y las horas dicen qué hacer esta tarde |
