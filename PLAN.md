...
| **Salud** | 116 pruebas en verde (79 servidor + 37 móvil) · TypeScript limpio en `server/` y `mobile/` · empaqueta para Android · `expo prebuild` acepta los plugins de LiveKit |
...
| 11.9 Cofres de 1K, 5K y 10K | ✅ | `lib/chests.ts` con premios por franjas. Los tres devuelven ~0,88 de media, por el mismo motivo que los regalos. Viven **dentro de la pestaña Suerte**, como tres casillas más delante de los regalos, igual que en Kako: es el mismo impulso que el regalo de la suerte y en el perfil quedaban lejos del momento de jugárselo. La escalera de premios y las reglas salen en «Detalles» (`components/chest-details.tsx`). **Verificación:** 8 pruebas, incluida una simulación de 200.000 aperturas por cofre |
